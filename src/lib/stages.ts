// Shared stage data with slug mapping
export interface Stage {
    id: number;
    title: string;
    icon: string;
    slug: string;
    variant?: 'default' | 'small'; // For "Temsili Sözleşme"
}

export const STAGES: Stage[] = [
    { id: 1, title: "Başvuru", icon: "folder_open", slug: "basvuru" },
    { id: 2, title: "Ön Teklif", icon: "upload_file", slug: "on-teklif" },
    { id: 3, title: "Kesin Teklif", icon: "check_box", slug: "kesin-teklif" },
    { id: 4, title: "Uzlaşma Görüşmeleri", icon: "groups", slug: "uzlasma-gorusmeleri" },
    { id: 5, title: "Temsili Sözleşme", icon: "star", slug: "temsili-sozlesme", variant: 'small' }, // New special stage
    { id: 6, title: "Karot Alımı", icon: "science", slug: "karot-alimi" },
    { id: 7, title: "Riskli Yapı İlanı", icon: "apartment", slug: "riskli-yapi-ilani" },
    { id: 8, title: "Sözleşme", icon: "signature", slug: "sozlesme" }, // Moved here
    { id: 9, title: "Tahliye Süreci", icon: "moving", slug: "tahliye-sureci" },
    { id: 10, title: "Ruhsat Alımı", icon: "article", slug: "ruhsat-alimi" },
    { id: 11, title: "Yıkım Süreci", icon: "domain_disabled", slug: "yikim-sureci" },
    { id: 12, title: "Anahtar Teslim", icon: "key", slug: "anahtar-teslim" },
];

export function getStageBySlug(slug: string) {
    return STAGES.find(s => s.slug === slug);
}

export function getStageById(id: number) {
    return STAGES.find(s => s.id === id);
}
